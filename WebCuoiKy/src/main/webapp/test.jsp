                ...
                <ul class="navbar-nav me-auto mb-2 mb-lg-0 bg-infor ">
                    <li class="nav-item dropdown dropstart"><a
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <% }else { %>
                            <li><a class="dropdown-item" href="#">Đơn hàng của người bình thường tôi</a></li>
                            <li><a class="dropdown-item" href="thaydoithongtin.jsp">Thay đổi thông tin</a></li>
                            <li><a class="dropdown-item" href="doimatkhau.jsp">Đổi mật khẩu</a></li>
                            <li><hr class="dropdown-divider"></li>
                            <li><a class="dropdown-item" href="dang-xuat">Đăng xuất</a></li>
...
// Get the list of products from the request
                    List<SanPham> listAllProduct = (List<SanPham>) request.getAttribute("listAllProduct");

// Check if the list is not null and not empty
                    if (listAllProduct != null && !listAllProduct.isEmpty()) {
                        // Use traditional for loop to iterate through the list
                        for (int i = 0; i < listAllProduct.size(); i++) {
                            SanPham p = listAllProduct.get(i);
                %>
                <!-- HTML structure for each product -->
                <div class="body_page-trending-product-list-card">
                    <div class="card-image">
                        <a href="product-detail?product_id=<%=p.getMaSanPham()%>"><img src="<%=p.getImage()%>" alt=""></a>
                    </div>
                    <div class="card-title-price">
                        <p><%= p.getTenSanPham() %></p>
                        <span style="font-size: 15px"><%= numberFormat.format(p.getGiaBan()) %></span>
                        <span style="margin-left: 10px; color: #6c6c6c; font-size: 15px">
                <strike><%= numberFormat.format(p.getGiaGoc()) %></strike>
            </span>
                    </div>
                    <div class="card-btn">
                        <button><a href="product-detail?product_id=<%=p.getMaSanPham()%>">Chi tiết</a></button>
                        <button onclick="addtocart(<%=p.getMaSanPham()%>)">Thêm vào giỏ</button>
                    </div>
                </div>
                <%
                        }
